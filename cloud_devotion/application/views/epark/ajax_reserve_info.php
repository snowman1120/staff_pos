...
<div class="inner">
    <div style="display: table; width: 100%; height: 40px;">
        ...
        <div style="float:right;">
            <button data="<?php echo $reserve['id']; ?>" pay-method="<?php echo empty($reserve['pay_method'])?'' : $reserve['pay_method']; ?>" id="btn_complete_order" class="btn_red" style="margin-left:5px;">完了</button>
        </div>
        <?php } ?>
        <?php if($reserve['is_reset_temp'] == true){ ?>
        <div style="float:right;">
            <button data="<?php echo $reserve['id']; ?>" pay-method="<?php echo empty($reserve['pay_method'])?'' : $reserve['pay_method']; ?>" id="btn_reset_order" class="btn_red" style="background-color: grey;">リセット</button>
        </div>
        <?php } ?>
    </div>
...
<script>
    ...
    function reset_order_temp(order_id){
        $.ajax({
